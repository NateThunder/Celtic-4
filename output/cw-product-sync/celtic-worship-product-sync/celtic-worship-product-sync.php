...
final class CW_Product_Sync {
    ...
    private static function configured(): bool {
        return defined('CW_PRODUCT_SYNC_URL') && defined('CW_PRODUCT_SYNC_SECRET') && CW_PRODUCT_SYNC_URL && CW_PRODUCT_SYNC_SECRET;
    }
    ...
    private static function send(array $payload): void {
        if (!self::configured()) throw new RuntimeException('CW_PRODUCT_SYNC_URL and CW_PRODUCT_SYNC_SECRET must be defined in wp-config.php.');
        $body = wp_json_encode($payload, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
        $timestamp = (string) time();
        $signature = hash_hmac('sha256', $timestamp . '.' . $body, CW_PRODUCT_SYNC_SECRET);
        $response = wp_remote_post(CW_PRODUCT_SYNC_URL, [
            'timeout' => 30,
    ...
    public static function admin_page(): void {
        ...
        echo '<p>Status: <strong>' . (self::configured() ? 'Configured' : 'Missing wp-config.php constants') . '</strong></p>';
