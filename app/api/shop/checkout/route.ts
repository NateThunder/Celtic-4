...
export async function POST(request: NextRequest) {
  ...
  let preparedCart;
  if (checkout.wallet_currency !== undefined) {
    if (checkout.payment_method !== SHOP_STRIPE_METHOD || !/^\d+$/.test(checkout.expected_total || "")) {
    ...
    if (String(quote.amount) !== checkout.expected_total || quote.currency !== checkout.wallet_currency) {
      return toProxyJsonResponse({ ...preparedCart, status: 409, payload: { message: "Your order total has changed. Please review it and open the wallet again." } });
    }
  }
