...
export type ShopCollageProps = {
  ...
  response?: number;
  /** multiplier applied to every plate's width and height */
  plateScale?: number;
  /** uniform greige tint applied to every plate */
  greigeTint?: number;
};
...
export default function ShopCollage({
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      for (const el of plates) {
        ...
        const t = Math.pow(smoothstep(clamp(1 - dist / focusWindow, 0, 1)), response);
  ...
  }, [baseOpacity, peakOpacity, maxBlur, focusWindow, response]);
  ...
  return (
    <>
      <div
        ...
        aria-hidden="true"
        style={{ ['--plate-tint' as string]: String(greigeTint) } as React.CSSProperties}
      >
        {LAYOUT.map((p, i) => (
          <div
            key={i}
            ...
            style={{
              left: `${p.x}vw`,
              top: `${p.y}%`,
              width: `${p.w * plateScale}vw`,
              height: `${p.w * plateScale * 1.25}vw`,
