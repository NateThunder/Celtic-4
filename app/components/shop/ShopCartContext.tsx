...
import { parseStoredCart } from "../../lib/browserCart";
...
let cartMigration: Promise<WooCartPayload> | undefined;
function loadCart(): Promise<WooCartPayload> {
  if (cartMigration) return cartMigration;
  cartMigration = (async () => {
    let payload = await requestCart("/api/shop/cart");
    const saved = parseStoredCart(localStorage.getItem("cw-shop-cart-v1"));
    for (const item of saved) {
      const existing = payload.items?.find((candidate) => candidate.id === item.id);
      // Use absolute quantities so a retry after an interrupted migration cannot duplicate items.
      payload = await requestCart(existing ? "/api/shop/cart/update-item" : "/api/shop/cart/add-item", {
        method: "POST", headers: { "Content-Type": "application/json" },
        body: JSON.stringify(existing
          ? { key: existing.key, quantity: Math.max(existing.quantity ?? 0, item.quantity) }
          : { id: item.id, quantity: item.quantity, variation: item.variation }),
      });
    }
    localStorage.removeItem("cw-shop-cart-v1");
    return payload;
  })().finally(() => { cartMigration = undefined; });
  return cartMigration;
}
