...
Before deploying the storefront, install and activate `wordpress/celtic-worship-cart-handoff.php` on the CMS (or copy it into `wp-content/mu-plugins/`). An installable ZIP is at `output/celtic-worship-cart-handoff.zip`. The default allowed browser origins are `https://celticworship.co.uk`, `https://www.celticworship.co.uk`, and the exact SiteGround staging origin `https://ifedadet1.sg-host.com`. Any other staging origin must be explicitly added through the `cw_cart_handoff_origins` WordPress filter. Do not allow arbitrary origins. Exclude `?wc-api=cw_cart_handoff` from page caching. No handoff secret belongs in the browser.
